...
func networkWithSellOrderObjects(t *testing.T, n int) (*network.Network, []types.SellOrder) {
	...
	for i := 0; i < n; i++ {
		sellOrder := types.SellOrder{
			SellOrderId: uint64(i),
		}
		nullify.Fill(&sellOrder)
		state.SellOrderList = append(state.SellOrderList, sellOrder)
	}
...
func TestShowSellOrder(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:          "found",
			idSellOrderId: objs[0].SellOrderId,
		...
		t.Run(tc.desc, func(t *testing.T) {
			args := []string{
				strconv.Itoa(int(tc.idSellOrderId)),
			}
...
func TestListSellOrder(t *testing.T) {
	net, objs := networkWithSellOrderObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListSellOrder(), args)
			require.NoError(t, err)
			var resp types.QueryAllSellOrderResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
